# CLI Track 2: `-ui` 기능 개발 명세

Track 1(CLI 핵심) 완료 이후에만 적용한다.

## 시작 조건

1. `docs/reference/specs/cli-core.md`의 완료 정의 충족
2. CLI 공통 계약(출력/오류/종료 코드) 고정
3. `-ui` 미구현 응답이 제거 가능한 상태

## 목표

- `pbdash -ui`를 실제 UI 실행 모드로 제공한다
- 로컬 HTTP 서버 + 정적 리소스 + 헬스체크를 안정적으로 제공한다
- Track 1 CLI 계약을 깨지 않고 UI 모드를 추가한다

## 범위

포함: `-ui` 플래그 실행, `-host/-port/-no-browser` 플래그, `/ /preview/* /styles/*` 라우팅, `/healthz` 200 응답, 브라우저 오픈 시도(실패 시 서버 유지)

제외: 원격 배포형 UI 서버, 서버 공유 세션, `/api/*` 백엔드 구현(V0는 404)
