...
이 디렉터리는 `pbdash`가 직접 사용하는 외부 의존성의 역할, 사용 맥락, 수정 시 확인 포인트를 정리한 문서 모음이다.

## 읽는 순서
- PocketBase 연동을 수정하거나 인증/조회 흐름을 봐야 하면 `pocketbase-client.md`를 먼저 본다.
- 그 외 터미널 UI, REPL, 테스트 보조 라이브러리는 이 문서에서 먼저 개요를 확인한다.

## 상세 문서
- `docs/development/dependencies/pocketbase-client.md`
...
- 사용 위치: `*_test.go`
- 현재 사용 방식
  - `assert`, `require`만 사용한다.
  - production code에는 사용하지 않는다.
- 주의사항
  - 테스트 표현을 간결하게 만드는 용도이며, 도메인 로직을 감추는 과한 helper 추상화는 피한다.
...
4. PocketBase SDK 관련 변경이라면 `pocketbase-client.md` 기준으로 인증, collections 조회, records 조회를 확인한다.
5. 출력 형식이나 에러 메시지 계약이 바뀌지 않았는지 확인한다.

## 현재 해석 기준
- 이 프로젝트의 핵심 외부 통합은 PocketBase SDK와 터미널 UI 스택이다.
- 간접 의존성 전체를 문서화하기보다, 실제로 직접 import 해서 프로젝트 동작에 영향을 주는 패키지 위주로 관리한다.
- 의존성 문서는 "패키지 소개"보다 "우리 코드에서 왜 쓰고 어디가 깨질 수 있는가"를 설명하는 데 집중한다.
